...
# Working directory
# wd = "M:/NANOK/Postprocessing/EndOfMay_Deliverable_v02/ArcGIS workflows/Arcpy/Playground/raw_data/21WXU"
wd = "M:/NANOK/Postprocessing/EndOfMay_Deliverable_v02/ArcGIS workflows/Arcpy/Playground/raw_data/21WXS"
 
#wd = arcpy.GetParameterAsText(0)
...
for file in os.listdir(wd):    
    if file.endswith('Land.shp'):
        Land = os.path.join(wd,file)    
    elif file.endswith('Intertidal.shp'):
        Intertidal = os.path.join(wd,file)
    elif file.endswith('Reef.shp'):
        Reef = os.path.join(wd,file)
    elif file.endswith('asible.shp'):
        NotFeasible = os.path.join(wd,file)

if not 'NotFeasible' in globals():
    NotFeasible = ""  

   
files = [Land,Intertidal,Reef,NotFeasible]

file_name = ['Land','Intertidal','Reef','NotFeasible']
i = 0
shapefiles = []
...
if not os.path.exists(os.path.join(dir_dissolve,'Merge.shp')):
    ...
    arcpy.Merge_management(shp_merge, os.path.join(dir_dissolve,'Merge'))
...
if not os.path.exists(os.path.join(dir_dissolve,'Smooth.shp')):
    ...
    arcpy.AddMessage(" Tolerance = " + tolerance + "meter")
